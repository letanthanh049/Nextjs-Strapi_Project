...
const ResetPassword = () => {
    ...
    const handleSubmit = async () => {
        ...
        const data = {
            ...
            passwordConfirm: document.querySelector('input[name="passwordConfirmation"]') as HTMLInputElement
        };
        const passInfo = {
            code: router.query.code,
            password: data['password'].value,
            passwordConfirmation: data['passwordConfirm'].value,
        };

        const resetPass = await fetch(`http://localhost:1337/api/auth/reset-password`, {
            method: "POST",
            headers: {
        ...
        if (typeof res.error !== "undefined") {
            console.log('Reset Pass error: ', res.error);
            swal({
                title: "Warning",
                    ...
                    <div className="md:flex w-full">
                        ...
                        <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
                            ...
                            <div>
                                ...
                                <div className="flex -mx-3">
                                    <div className="w-full px-3 mb-12">
                                        <label htmlFor="" className="text-xs font-semibold px-1">Password Confirmation:</label>
                                        <div className="flex">
                                            <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                                            </svg>
                                            </div>
                                            <input type="password" name="passwordConfirmation" className="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="************" />
                                        </div>
                                    </div>
                                </div>
                                <div className="flex -mx-3">
                                    <div className="w-full px-3 mb-5">
                                        <button onClick={() => handleSubmit()} className="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">OKAY</button>
                                    </div>
                                </div>
                            </div>
